...
perl -i -pe 's/^#?((?!Password|Kerberos|GSSAPI)\w*Authentication)\s.*/\1 no/; s/^(PubkeyAuthentication) no/\1 yes/' /etc/ssh/sshd_config
# Disable sftp subsystem
perl -i -pe 's/^(Subsystem\ssftp\s)/#\1/' /etc/ssh/sshd_config

if [ "$SSH_DEBUG" == 'true' ]; then
  # Set logging level
  perl -i -pe 's/^#?(LogLevel) \w*/\1 DEBUG/' /etc/ssh/sshd_config
fi
